...
class OnBrowserStartupControl(BaseModel):
    """
    Control the browser's 'On Startup' settings
    """ # noqa: E501
    action: OnBrowserStartupControlAction
    urls_to_open: Optional[Annotated[List[Annotated[str, Field(min_length=1, strict=True, max_length=2048)]], Field(max_length=50)]] = Field(default=None, description="URLs opened on startup or after restoring the previous session. Required and must be non-empty when action is openUrls or restorePreviousSessionAndOpenUrls; must not be set for other actions. Each entry must be an http(s)/ftp URL, a schemeless host (e.g. example.com), a browser page (chrome:// or prisma://), or a file:// URL. Dangerous schemes (javascript:, data:, vbscript:) and other custom schemes are rejected.", alias="urlsToOpen")
    recommended: Optional[StrictBool] = Field(default=None, description="Whether this setting is recommended and user-changeable.")
    __properties: ClassVar[List[str]] = ["action", "urlsToOpen", "recommended"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of OnBrowserStartupControl from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of OnBrowserStartupControl from a dict"""
        if obj is None:
            return None
