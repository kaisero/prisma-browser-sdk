...
class RequestDesktopMobileSiteControlRulesInner(BaseModel):
    """
    RequestDesktopMobileSiteControlRulesInner
    """ # noqa: E501
    domain: Annotated[str, Field(min_length=1, strict=True, max_length=2048)] = Field(description="Match target the browser compares against visited URLs to apply the site-mode override. Accepts a hostname/domain (optionally with \"*.\" or \"*\" wildcards, including a bare \"*\" catch-all for all sites), or a \"scheme://host[:port]/path\" match pattern. Must be non-empty and contain at least one letter, digit, or \"*\". Percent-encoding, fragments, and the path/query punctuation real match patterns use are admitted; whitespace and markup/injection characters that never appear in a host or match pattern are rejected.")
    mode: RequestDesktopMobileSiteControlRulesInnerMode
    ...
    def domain_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^[A-Za-z0-9._:\/*?=&%#~+@,;-]*[A-Za-z0-9*][A-Za-z0-9._:\/*?=&%#~+@,;-]*$", value):
            raise ValueError(r"must validate the regular expression /^[A-Za-z0-9._:\/*?=&%#~+@,;-]*[A-Za-z0-9*][A-Za-z0-9._:\/*?=&%#~+@,;-]*$/")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RequestDesktopMobileSiteControlRulesInner from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RequestDesktopMobileSiteControlRulesInner from a dict"""
        if obj is None:
            return None
