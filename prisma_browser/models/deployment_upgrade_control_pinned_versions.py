...
class DeploymentUpgradeControlPinnedVersions(BaseModel):
    """
    Pinned target versions by desktop OS. Required when mode is pinned. Must not be set for other modes.
    """ # noqa: E501
    mac: Annotated[str, Field(min_length=1, strict=True, max_length=50)] = Field(description="Pinned macOS browser version")
    windows: Annotated[str, Field(min_length=1, strict=True, max_length=50)] = Field(description="Pinned Windows browser version")
    __properties: ClassVar[List[str]] = ["mac", "windows"]
    ...
    def mac_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^\d+(\.\d+){0,3}$", value):
            raise ValueError(r"must validate the regular expression /^\d+(\.\d+){0,3}$/")
        return value

    @field_validator('windows')
    def windows_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not isinstance(value, str):
            value = str(value)
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of DeploymentUpgradeControlPinnedVersions from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of DeploymentUpgradeControlPinnedVersions from a dict"""
        if obj is None:
            return None
