...
class ManagedShortcutsControl(BaseModel):
    """
    Set the applications that appear on the new tab page and in bookmarks.
    """ # noqa: E501
    mode: ManagedShortcutsControlMode
    shortcuts: Optional[Annotated[List[ManagedShortcutsControlShortcutsInner], Field(min_length=1, max_length=100)]] = Field(default=None, description="Managed shortcuts shown on the new tab page and in bookmarks. Required when mode is setShortcuts. Must not be set when mode is noShortcuts. At most 16 entries may have `pinned: true`; requests that exceed this cap are rejected with HTTP 400. ")
    __properties: ClassVar[List[str]] = ["mode", "shortcuts"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ManagedShortcutsControl from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ManagedShortcutsControl from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "mode": obj.get("mode"),
            "shortcuts": [ManagedShortcutsControlShortcutsInner.from_dict(_item) for _item in obj["shortcuts"]] if obj.get("shortcuts") is not None else None
        })
        return _obj
